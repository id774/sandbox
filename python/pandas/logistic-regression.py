...
def plot_x_by_y(x, y, colors, ax=None):
    ...
    if ax is None:
        ...
        # Add Axes to the drawing area and adjust the margins
...
def conversion(data):
    ...
    x = data[columns]    # data, the explanatory variables
    y = data['Species']  # labels, the response variable

    # Convert the labels into 0 and 1 columns
...
def main(args):
    ...
    # Keep only setosa and versicolor to make this a two class problem
    data = iris[:100]
    x, y = conversion(data)
    # Two explanatory variables, so two dimensions
    print(y)
