# -*- coding: utf-8 -*-
# Estimate naive Bayes parameters for the multivariate Bernoulli model.
...
def mm_make_classifier(V, classes, p_c, q_cw):
    ...
    def classify(words):
        p_given_c = [log(p_c[c]) + sum([log(q_cw[c][w])
                                        for w in words if w in q_cw[c] and q_cw[c] > 0]) for c in classes]
        return argmax(p_given_c)
    return classify
...
# Estimate the parameters, here by MAP on the multivariate Bernoulli model
