# Compare and time two ways of drawing Poisson random numbers.
...
tm = timeit.Timer("poisson_rand1(%d)" %
                  mu, "from __main__ import poisson_rand1")
time.append(tm.timeit(N))
time2.append(-1)

s.append([poisson_rand2(mu) for x in range(N)])
t.append("poisson_rand2 (mu=%d)" % mu)
tm = timeit.Timer("poisson_rand2(%d)" %
                  mu, "from __main__ import poisson_rand2")
time.append(tm.timeit(N))
time2.append(-1)
...
time.append(tm.timeit(N))
tm = timeit.Timer("numpy.random.poisson(%d,%d)" %
                  (mu, N), "import numpy.random")
...
time.append(tm.timeit(N))
tm = timeit.Timer("scipy.stats.poisson.rvs(%d, size=%d)" %
                  (mu, N), "import scipy.stats")
time2.append(tm.timeit(1))
...
for i in range(4):
    ...
    text(19, 0.16, "%3.3fmsec" %
         (time[i] * 1000), color='gray', fontsize=12, horizontalalignment='right')
    if time2[i] > 0:
        text(19, 0.15, "%3.3fmsec" % (
            time2[i] * 1000), color='gray', fontsize=12, horizontalalignment='right')
    title(t[i])
